...
public class code337 {
    ...
    public int dfs(TreeNode root,Map<TreeNode,Integer> map){
        ...
        int money = root.val;
        if (root.left != null) money += dfs(root.left.left,map) + dfs(root.left.right,map);
        if (root.right != null) money += dfs(root.right.left,map) + dfs(root.right.right,map);

        int result = Math.max(money,dfs(root.left,map) + dfs(root.right,map));
        map.put(root,result);
        return result;
    }
}
